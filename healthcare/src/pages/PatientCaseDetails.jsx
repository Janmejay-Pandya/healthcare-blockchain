        ...
        <div className="min-h-screen bg-gray-900 text-gray-100 p-6">
            <div className="max-w-4xl mx-auto bg-gray-800 p-6 rounded-lg shadow-lg">
                {loading ? (
                    ...
                    <>
                        <h2 className="text-2xl font-bold text-teal-500 mb-4">{caseDetails.caseTitle}</h2>
                        <p className="text-gray-400 mb-4">Case ID: #{caseDetails.caseId}</p>
                        <p className={`mb-4 ${caseDetails.isOngoing ? "text-green-400" : "text-red-400"}`}>
                            ...
                            <div className="mb-6">
                                {records.map((record, index) => (
                                    <div key={index} className="bg-gray-700 p-4 rounded-lg mb-4">
                                        ...
                                        <p className="text-sm text-gray-400 mb-2">Doctor: {record.doctor}</p>
                                        {record.symptoms && <p><span className="font-semibold text-teal-400">Symptoms:</span> {record.symptoms}</p>}
                                        {record.cause && <p><span className="font-semibold text-teal-400">Cause:</span> {record.cause}</p>}
                                        {record.inference && <p><span className="font-semibold text-teal-400">Inference:</span> {record.inference}</p>}
                                        {record.prescription && <p><span className="font-semibold text-teal-400">Prescription:</span> {record.prescription}</p>}
                                        {record.advices && <p><span className="font-semibold text-teal-400">Advice:</span> {record.advices}</p>}
                                        {record.medications && <p><span className="font-semibold text-teal-400">Medications:</span> {record.medications}</p>}
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p className="text-gray-400 mb-6">No medical records found for this case.</p>
                        )}

                        <h3 className="text-xl font-semibold text-gray-300 mb-4">Medical Reports</h3>
                        {caseDetails.reportCIDs && caseDetails.reportCIDs.length > 0 ? (
                            <ul className="list-disc pl-5 mb-6">
                                {caseDetails.reportCIDs.map((cid, index) => (
                                    <li key={index} className="text-gray-400">
                                        CID: {cid}
                                        {/* You might want to add functionality to view these reports if they are accessible via IPFS or a similar system */}
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-gray-400 mb-6">No medical reports found for this case.</p>
                        )}
